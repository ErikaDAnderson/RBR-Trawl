#=====================================================================================================
# Script Name: FranklinTesting2020-03.R
# Modified from TowPlots.R in GearComparison2018-10 project
#
...
# Update: Modified script to produce plots for Franklin fishing trials
#
...
dataFolder <- here::here("Input/2020-03-FranklinFishingTrials")
...
Event_input <- here::here("Input/2020-03-FranklinFishingTrials/bridge_202003.xls")
net_input <- paste0(getwd(), "/Output/2020-03-FranklinFishingTrials/Bridge201876.csv")
...
plot_path <- paste0(figFolderName,"/TowPlots")

#create new folder if it does not exist yet for today's date
...
event_num <- read_xls(Event_input, skip = 2)
event_num <- subset(event_num, select = c(1:8, 23:25, 27))

# fix names

# strip incorrect dates that were autogenerated on load
...
event_num <- filter(event_num, toupper(Type) == "TRAWL")

# make date time columns for start and stop using GF_Bio columns names
event_num <- event_num %>%
  mutate(., FE_END_DEPLOYMENT_TIME = ymd_hms(paste0(Date, StartTime)),
         FE_BEGIN_RETRIEVAL_TIME = ymd_hms(paste0(Date, StopTime))) %>%
  rename(FE_MAJOR_ID = Event, TRLSP_WARP_LENGTH = Warp)
...
ggplot(data = event_num, aes(x = FE_END_DEPLOYMENT_TIME, y = TowTime)) +
...
plotting_DF <- subset(plotting_DF, select = c(2, 9:14))
...
plotting_DF_sum <- plotting_DF %>%
  ...
  summarise(RbrVerticalNet = round(mean(Opening), 1)) %>%
  ungroup()

# save to pull into stats Rscript
write.csv(plotting_DF_sum, 
          str_c("Output/RBRopenings", Sys.Date(), ".csv", sep = ""), 
          row.names = FALSE)
